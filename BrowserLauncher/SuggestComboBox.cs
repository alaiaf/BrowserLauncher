...
namespace BrowserLauncher
{
    public class SuggestComboBox : ComboBox
    {
        ...
        private int _maxSuggestBoxHeight;
        private const int SUGGESTION_ENTRY_HEIGHT= 16;
        private const int SUGGESTION_ENTRY_OFFSET = 4;

        public int MaxSuggestBoxHeight
        {
            get { return _maxSuggestBoxHeight; }
            set { if (value >= SUGGESTION_ENTRY_HEIGHT) _maxSuggestBoxHeight = value; }
        }
        ...
        public int SuggestBoxHeight
        {
            ...
            set
            {
                if (value > 0)
                {
                    int suggHeight = SUGGESTION_ENTRY_HEIGHT + SUGGESTION_ENTRY_OFFSET;
                    if (value <= suggHeight)
                    {
                        _suggLb.Height = suggHeight;
                    }
                    else
                    {
        ...
        protected override void OnTextChanged(EventArgs e)
        {
            ...
            if (_suggBindingList.Any())
            {
                int tempHeight = _suggBindingList.Count * SUGGESTION_ENTRY_HEIGHT;
                if (_suggBindingList.Count >= 1)
                {
                    if (tempHeight < MaxSuggestBoxHeight)
                    {
                        SuggestBoxHeight = tempHeight; 
                    }
                    else
                    {
                        SuggestBoxHeight = MaxSuggestBoxHeight;
                    }
                }
                ...
                else if (_suggBindingList.Count == Items.Count)
                {
                    _suggLb.Visible = false;
                }
                else
